...
def test_extracted_theorem_interaction_in_gym() -> None:
    for i in range(0, 2):
        logger.debug(f"i: {i}")
        code = read_code_from_file("./tests/data/Mathlib.Order.Bounds.Basics_rewrite.lean")
        ...
        if i == 0:
            ...
            tracer.load_trace_result(tracer.execution_env.tmp_dir / "build/ir/Main.ast.json")
        # Get tactics
