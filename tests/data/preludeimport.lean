...
theorem Nat.lt_add_of_zero_lt_left (a b : Nat) (h : 0 < b) : a < a + b := by exact (
  show a + 0 < a + b by
    apply Nat.add_lt_add_left
    assumption)
#align nat.lt_add_of_zero_lt_left Nat.lt_add_of_zero_lt_left

theorem Nat.zero_lt_one_add (a : Nat) : 0 < 1 + a := by exact (
  suffices 0 < a + 1 by
    simp [Nat.add_comm]
    assumption
  Nat.zero_lt_succ _)
...
theorem Nat.lt_add_left (a b c : Nat) : a < b → a < c + b :=
  by
  lean_dojo_repl
  sorry
